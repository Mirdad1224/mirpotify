...
const Player: FC = () => {
  ...
  const [volume, setVolume] = useState(
    Number(localStorage.getItem("mirpotify-volume")) || 0.5
  );
  const [isMuted, setIsMuted] = useState(
    !!Number(localStorage.getItem("mirpotify-muted")) || false
  );
  ...
  useEffect(() => {
    ...
    localStorage.setItem("mirpotify-volume", String(volume));
    localStorage.setItem("mirpotify-muted", String(+isMuted));
  }, [volume, isMuted]);
